...
$address = "consultas@sucesion.org";
...
if(empty($token)) {
    $error = 'Attention! Invalid captcha token.';
}

if(!verifyCaptcha($token)){
    $error = 'Attention! Invalid captcha.';
}
...
if (!empty($error)) {
 ...
 if(mail($address, $e_subject, $msg, $headers)) {
	 ...
	 echo "<h1>Su mensaje ha sido enviado!</h1>";
	 echo "<p>Gracias <strong>$name</strong>, responderemos su mensaje a la brevedad.</p>";
	 echo "</div>";
	 echo "</fieldset>";
...
{
    $secretKey = "6LfClv8ZAAAAADXqYhVIYMLBhBiJ_VJUGGDp21tk";
